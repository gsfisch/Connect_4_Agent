...
class ConnectFourEnv(gymnasium.Env):
    ...
    def _wait_for_render(self):
        ...
        if (
            self.last_render_time is not None
            and current_time - self.last_render_time < time_to_wait
        ):
    ...
    def _render_frame(self):
        ...
        windows_width = (
            (padding * 2)
            + (circle_radius * 2 * self.COLUMNS_COUNT)
            + padding_center * (self.COLUMNS_COUNT - 1)
        )
        end_height_board = (
            (padding * 2)
            + (circle_radius * 2 * self.ROWS_COUNT)
            + padding_center * (self.ROWS_COUNT - 1)
        )
        ...
        for i in range(self.ROWS_COUNT):
            ...
            for j in range(self.COLUMNS_COUNT):
                ...
                pygame.draw.circle(
                    canvas,
                    color,
                    (j_position + circle_radius, i_position + circle_radius),
                    circle_radius,
                )
        ...
        pygame.draw.circle(
            canvas,
            self.player_2_color,
            (50, text_position_y_first_player + circle_radius / 4),
            circle_radius / 2,
        )
        font = pygame.font.Font(None, 36)
        opponent_name = (
            self._opponent.getName() if self._opponent is not None else "Opponent"
        )
        ...
        pygame.draw.circle(
            canvas,
            self.player_1_color,
            (50, text_position_y_second_player + circle_radius / 4),
            circle_radius / 2,
        )
